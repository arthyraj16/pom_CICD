<# START USER INPUTS#>
# UPDATED: Reads API Key from GitHub Environment Variable
...
function checkTestPlanRunStatus{
  ...
  for($i=0; $i -le $NO_OF_POLLS;$i++){
    ...
    if ($EXECUTION_STATUS -eq "STATUS_IN_PROGRESS" -or $EXECUTION_STATUS -eq "STATUS_QUEUED"){
...
function saveFinalResponseToAFile{
  if ($IS_TEST_RUN_COMPLETED -eq 0){
     ...
     Write-Host "Report data downloaded."
